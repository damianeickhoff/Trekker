...
export function MobileTop({
  ...
  return (
    <header
      className={`relative z-(--z-top-row) flex h-[71px] shrink-0 items-center justify-between px-5 pt-[27px] lg:hidden ${onHero ? "text-white" : ""}`}
    >
      {title === "wordmark" ? (
...
export function BackHeader({ back, title, meta }: { back: ReactNode; title: ReactNode; meta?: ReactNode }) {
  return (
    <header className="flex flex-col gap-2 lg:gap-4">
      <div className="flex h-[71px] items-center pt-[27px] lg:h-auto lg:pt-0">{back}</div>
