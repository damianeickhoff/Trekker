...
| Rows wash under the pointer | `ROW_WASH` in `motion.ts`: Also waiting, notifications, search, friends, Most watched, the calendar's day lists; history, agenda and habit tiles lift their fill (`hover:bg-surface-2`) | `--fast` | `--ease-out` | a pseudo-element's opacity or a fill fade; no movement |
